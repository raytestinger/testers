...
# CPAN server to get updated modules from
# this must be in the urllist of the MyConfig.pm for the user's CPAN
...
my $smoke_test_modules_url = "http://cpan.cpantesters.org/modules/01modules.index.html";

my $function_cpanm_home = "$tester_function/.cpanm";
my $function_cpanmreporter_home = "$tester_function/.cpanmreporter";
...
my $Modules = get($smoke_test_modules_url);
...
### http://www.dagolden.com/index.php/2098/the-annotated-lancaster-consensus
            ...
            $module =~ s/-/::/g;
	    chomp ($module);	# it happens with AAKHTER/Visio
