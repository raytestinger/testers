...
my $cpanmreporter_reports_sent  = "$ENV{HOME}/.cpanreporter/reports-sent.db";
...
sub do_smoke {
    ...
    while (<modules_fh>) {
        ...
        if ( ( $module =~ /^ href=/ ) && ( $module =~ /\.tar\.gz/ ) ) {
            ...
            unless ( scalar @fields eq 7 )
            {    # some modules have extra / in entry
		say "$module" if ($verbose);
                say
"number of directories in module entry is not 7, skip this entry"
                  if ($verbose);
                next;
            }

	    if (scalar @fields eq 7) {
            $fields[6] = substr $fields[6], 0, rindex( $fields[6], '-' );
            $module = $fields[5] . "/" . $fields[6];
	    }

	    if (scalar @fields eq 8) {
            $fields[7] = substr $fields[7], 0, rindex( $fields[7], '-' );
            $module = $fields[5] . "/" . $fields[7];
	    }
...
# called only if $function = 'recent'
