...
my $function_cpanmreporter_home = "$ENV{HOME}/testers/$function/.cpanmreporter/bin";
...
"perlbrew exec --with $perlbuild cpanm --test-only $module | tee $function_test_log/$module.$perlbuild"
	    ...
	    $ENV{PERL_CPAN_REPORTER_DIR} = '/home/larry/.cpanreports';
...
#           $command .= "$CPANM_REPORTER_HOME/cpanm-reporter ";
            ...
            $command .= "| tee $function_reporter_log/$module.$perlbuild ";
