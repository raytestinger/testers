...
while ( @jobs || $pm->running_procs ) {
    ...
    if ( $pm->running_procs >= $pm->max_procs || !@jobs ) {
        ...
        # timeout is between 120 and 150 seconds.
        sleep $TIMEOUT * 0.25;
        next;
    }
...
LOOP:
    ...
    chomp($tested);
    if ( (defined $tested) && ($tested > 0 )) {
        say "$module already tested, skip it" if ($verbose);
	next;
    }
    my $pid = $pm->start and next LOOP;
    say "After starting job";
    say Dumper $pm;
    say "Before adding to watchdog timer" if ($verbose);
    say Dumper %watching;
    ...
    say "After adding to watchdog timer" if ($verbose);
    say Dumper %watching;
        next;
    }
